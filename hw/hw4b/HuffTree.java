...
public class HuffTree<Key, E> {
    ...
    public void buildCode(BinNode<Integer, String> a)
    {
        String value = "";
        if(a.getLeft().isLeaf() == true)
        {
            codeTable.put(a.getLeft().getValue(), value + "0");
            buildCode(root.getRight());
            value = value + "1";
        }
        else
        {
            if(a.isLeaf() == true)
            {
                codeTable.put(a.getValue(), value + "1");
            }
            else
            {
                value = value + "1";

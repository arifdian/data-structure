...
public class MyGLSurfaceView extends GLSurfaceView {
    ...
    public boolean onTouchEvent(MotionEvent pEvent) {
        ...
        if(pEvent.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            System.out.println("C343 hw03 ---------------------------------- ");
            try
            {
                ContactCollection tc = new ContactCollection((android.app.Activity)gContext);
                String[] separate = tc.readContactsFile().split(" ");
                int i = 0;
                while(i < separate.length)
                {
                    tc.contact.add(new Contact(separate[i],separate[i+1],separate[i+2]));
                    i += 3;
                }
                Contact one = tc.contact.get(2);
            ...
            {
                System.out.println("the ContactCollection threw: " + e + " exception.");
            }

...
public class BSTJr <K extends Comparable<?super K>>
{
    BinNode<K> root;
    BinNode<K> curr;
    //     ( yesterday, for Lab 08, you used "unbalanced" for balance checking:
    ...
    public String select(int kth)
    {
        if(kth <= root.getSize())
        {
            if (kth == root.getLeft().getSize() + 1 || root.getLeft() == null)
            {
                return root.getKey() + "";
            }
            else if (kth < root.getLeft().getSize() + 1)
            {
                root = root.getLeft();
            }
            else if (kth > root.getLeft().getSize() + 1)
        ...
        {
            System.out.println("kth value not found in tree");
        }
        return "";
    }

    // TODO for C343/Summer 2017 - Lab 09 :
    //  you may write a helper function select(node, kth) here
    //  hint: use recursion
    ...
    public static void main(String[] argv) {
        ...
        tree.build(ks);
        tree.display();
        System.out.println(tree.root.getLeft().getLeft().getLeft().getSize());
        System.out.println("3rd smallest = " + tree.select(3));
        System.out.println(tree.select(1000));
    }
}
